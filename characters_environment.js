...
function Play() {
  gameStatus = "status"
  document.getElementById("Jogocarregado").innerHTML = "Jogo carregando"
}
...
function game(){
  ...
  if(gameConfig.status==='start'){
    ...
    textSize(40);
    textAlign(CENTER);
    text("mova sua nariz para a posição quer o mario deve ir", gameConfig.screenX/2, gameConfig.screenY/2);
    textSize(40);
  ...
  if(gameConfig.status==='gameover'){
    ...
    textSize(15);
    text("Chege seus pulso direito perto da camera para reiniciar", gameConfig.screenX/2, gameConfig.screenY/2+135);
    textSize(40);
...
function changeGameStatud(character){
  if((gameStatus == "status")&& gameConfig.status==="start") {
    world_start.play();
    initializeCharacterStatus(mario);
    gameConfig.status= "play";
  }
  if(gameConfig.status==="gameover" && rightWrist > 0.2) {
    gameConfig.scores = 0;
    character.coins = 0;
    character.kills = 0;
...
function jumping(character){
	if( (noseY < 200) || (touchIsDown&&character.live) ){
